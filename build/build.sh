...
#     eg: "v1.1.1" will name the bundle "rac_size_matters_apworld-v1.1.1"
...
function mk_apworld() {
    ...
    echo "From: ${root}"
    echo "To: ${destdir}"
    mkdir --parents "${destdir}/rac_size_matters"
    rsync --progress \
        --recursive \
        --prune-empty-dirs \
        --exclude-from="${CWD}/apworld.ignore" \
        "${root}/" "${destdir}/rac_size_matters"

    echo "${tag}" > "${destdir}/rac_size_matters/version.txt"

    pushd "${destdir}"
    zip -9r "rac_size_matters.apworld" "rac_size_matters"
    popd

    rm --force --recursive "${destdir}/rac_size_matters"
}
...
function main() {
    ...
    local bundle_base="rac_size_matters_apworld"
